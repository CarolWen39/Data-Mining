...
import sys, csv, itertools, collections, time, re
...
def SON_Pass1(iterator):
        ...
        sub_threshold = ceil((len(sub_basket) / baskets_num) * float(threshold))
...
if __name__ == '__main__':
    ...
    '''Data Pre-processing'''
    cvsRDD = sc.textFile(input_filepath).filter(lambda item: not item.startswith('"TRANSACTION_DT"')) \
        .map(lambda id: id.split(',')).map(lambda kv: (kv[0].strip('"')[0:5]+kv[0].strip('"')[7:9], kv[1].strip('"').lstrip('0'), kv[5].strip('"').lstrip('0')))
    data = cvsRDD.collect()
    
    mergeRDD = cvsRDD.map(lambda kv: (kv[0]+'-'+kv[1], int(kv[2]))).groupByKey().mapValues(list).map(lambda kv: (kv[0], kv[1]))
    merge_data = mergeRDD.collect()    
    
    data_after_propocess = [('DATE-CUSTOMER_ID','PRODUCT_ID')]
    
    for item in merge_data:
        s = ','.join(str(i) for i in item[1]) + '\t'
        data_after_propocess += [(item[0], s)]
    
    # save cvs file
    file =  open('Customer_product.csv', 'w+')
    writer = csv.writer(file, dialect = 'excel')
    writer.writerows(data_after_propocess)
    file.close()

    '''Apply SON Algorithm'''
    start_time = time.time()

    basketsRDD = sc.textFile('Customer_product.csv').filter(lambda id: not id.startswith('DATE-CUSTOMER_ID')) \
        .map(lambda id: re.split('"|\t', id)).map(lambda kv: set(kv[1].split(','))) \
        .filter(lambda kv: len(kv) > int(filter_threshold)).cache()
    ...
    frequence_itemsets = basketsRDD.mapPartitions(SON_Pass2).reduceByKey(add) \
        .filter(lambda kv: kv[1] >= int(threshold)) \
